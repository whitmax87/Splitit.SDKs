...
class CreateInstallmentPlanApi
{
    ...
    protected function injectSessionRequestHeaders($request)
    {
        if (!is_null($this->sessionId)){
            if (is_null($request->getRequestHeader())){
                $request->setRequestHeader(new RequestHeader());
            }

            if (is_null($request->getRequestHeader()->getSessionId())){
                $request->getRequestHeader()->setSessionId($this->sessionId);
            }

            if (is_null($request->getRequestHeader()->getApiKey())){
                $request->getRequestHeader()->setApiKey($this->config->getApiKey());
            }
        }
    }
