"use strict";
/* tslint:disable */
/* eslint-disable */
/**
 * splitit-web-api-public-sdk
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 1.0.0
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
Object.defineProperty(exports, "__esModule", { value: true });
/**
 *
 * @export
 * @enum {string}
 */
var IntegrationType;
(function (IntegrationType) {
    IntegrationType["None"] = "None";
    IntegrationType["Shopify"] = "Shopify";
    IntegrationType["Woocommerce"] = "Woocommerce";
})(IntegrationType = exports.IntegrationType || (exports.IntegrationType = {}));
function IntegrationTypeFromJSON(json) {
    return IntegrationTypeFromJSONTyped(json, false);
}
exports.IntegrationTypeFromJSON = IntegrationTypeFromJSON;
function IntegrationTypeFromJSONTyped(json, ignoreDiscriminator) {
    return json;
}
exports.IntegrationTypeFromJSONTyped = IntegrationTypeFromJSONTyped;
function IntegrationTypeToJSON(value) {
    return value;
}
exports.IntegrationTypeToJSON = IntegrationTypeToJSON;
